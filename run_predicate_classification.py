...
# Required parameters
flags.DEFINE_string(
    "data_dir", model_config.PREDICATE_DATA_OUTPUT_DIR,
    "The input data dir. Should contain the .tsv files (or other data files) "
    "for the task.")

flags.DEFINE_string(
    "bert_config_file", os.path.join(model_config.MODEL_DIR, 'bert_config.json'),
    "The config_ json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("task_name", 'ske_2019', "The name of the task to train.")

flags.DEFINE_string("vocab_file", os.path.join(model_config.MODEL_DIR, 'vocab.txt'),
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", model_config.PREDICATE_CHECKPOINT,
    "The output directory where the model checkpoints will be written.")

# Other parameters

flags.DEFINE_string(
    "init_checkpoint", os.path.join(model_config.MODEL_DIR, 'bert_model.ckpt'),
...
flags.DEFINE_bool("do_inference", True, "Whether to run the model in inference mode on online inference.")

flags.DEFINE_string('export_dir_base', model_config.PREDICATE_PB_MODEL,
...
class SKE_2019_Multi_Label_Classification_Processor(DataProcessor):
    ...
    def get_examples(self, data_dir):
        ...
        获取数据样例
        :param data_dir:
        :return: list，[(tokens, predicate_label),...]
        """
    ...
    def get_labels(self):
        return model_config.PREDICATE_LABEL

    def _create_example(self, lines, set_type):
        """Creates examples for the training and dev sets.
        :arg
        lines: list，[(tokens, predicate_label),...]
        set_type: str, "train" or "valid"

        :return
        list, [InputExample1, InputExample2, ...]
        """
        examples = []
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, output_file):
    ...
    tokenizer: obj, bert tokenizer
    output_file: str, tf_record 的文件地址
    """
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    :arg
    bert_config:
    input_ids: tensor 输入,shape= [batch_size, seq_len, hidden_size]
    """
    ...
    with tf.variable_scope("loss"):
        ...
        if not FLAGS.do_inference:
            label_ids = tf.cast(labels, tf.float32)
            # 计算每个样本的loss， shape = [batch_size]
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    :arg
    bert_config: dict, bert_config.json中的模型参数
    num_labels: int, 标签的数据量
    init_checkpoint: str, 预训练模型存储位置
    learning_rate: float, 学习率
    num_train_steps: int, 总的训练步数
    num_warmup_steps: int, 总的学习率衰减步数
    use_tpu: bool, 是否使用tpu
    use_one_hot_embeddings: bool, 是否使用one-hot表示
    """
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if "is_real_example" in features and FLAGS.do_eval:
            is_real_example = tf.cast(features["is_real_example"], dtype=tf.float32)
        elif "is_real_example" not in features and FLAGS.do_eval:
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, probabilities, is_real_example):
                ...
                评估函数
                """
                # 把probabilities大于0.5的数值变成1，否则变成0
                predict_ids = tf.cast(probabilities > 0.5, tf.int32)
                label_ids = tf.cast(label_ids, tf.int32)
                # 对应位置比较，相同变为1，不同变为0， shape = [batch_size, label_nums]
        ...
        if FLAGS.do_inference:
            ...
            predictions_dict = {"predictions": probabilities}
            export_outputs = {
...
def main():
    ...
    if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.do_inference:
        raise ValueError(
            "At least one of `do_train`, `do_eval` , 'do_inference' or `do_predict' must be True.")
        ...
        with tf.gfile.GFile(output_score_value_file, "w") as score_value_writer:
            with tf.gfile.GFile(output_predicate_predict_file, "w") as predicate_predict_writer:
                ...
                for (i, prediction) in enumerate(result):
                    ...
                    output_line_score_value = " ".join(str(class_probability)
