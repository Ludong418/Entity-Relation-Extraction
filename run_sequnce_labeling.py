...
# Required parameters
flags.DEFINE_string(
    "data_dir", model_config.SEQUENCE_DATA_OUTPUT_DIR,
...
flags.DEFINE_string(
    "output_dir", model_config.SEQUENCE_CHECKPOINT,
...
flags.DEFINE_string('export_dir_base', model_config.SEQUENCE_PB_MODEL,
...
def convert_single_example(ex_index, example, token_label_list, predicate_label_list, max_seq_length, tokenizer):
    ...
    把句子和对应的序列转换：
    输入是 句子 + [SEP] + 关系标签 * 句子长度
    序列标签是 对应序列 + [SEP] + [category] * 句子长度
    转换目的：
    输入句子与关系类别进行绑定，让模型能学习标注序列和关系（predicate）之间的联系，
    [category]可以理解为补充序列长度的目的，使得能和句子对齐，在预测时这部分结果是没有用的
    """
...
def file_based_convert_examples_to_features(
    ...
    :arg
    examples: list, [InputExample1, InputExample2, ...]
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    loss, predicate_loss, predicate_per_example_loss, token_label_loss, \
    token_label_per_example_loss, predicate_prediction = 0, 0, 0, 0, 0, 0
    ...
    if not FLAGS.do_inference:
        token_label_predictions = token_label_probabilities
...
def model_fn_builder(bert_config,num_token_labels, num_predicate_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if FLAGS.do_train or FLAGS.do_eval or FLAGS.do_predict:
            ...
            token_label_ids, predicate_label_id = None, None

        is_real_example = None
        if "is_real_example" in features and FLAGS.do_eval:
            is_real_example = tf.cast(features["is_real_example"], dtype=tf.float32)
        elif "is_real_example" not in features and FLAGS.do_eval:
            is_real_example = tf.ones(tf.shape(token_label_ids), dtype=tf.float32)

        is_training = (mode == tf.estimator.ModeKeys.TRAIN)

        (total_loss, predicate_loss, predicate_per_example_loss, predicate_probabilities, predicate_prediction,
        ...
        if FLAGS.do_inference:
            ...
            predictions_dict = {
                "predicate_predictions": predicate_probabilities,
                "token_label_predictions": token_label_predictions}
            export_outputs = {
...
def main():
    ...
    if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.do_inference:
        raise ValueError(
            "At least one of `do_train`, `do_eval`, `do_predict' or 'do_inference' must be True.")
